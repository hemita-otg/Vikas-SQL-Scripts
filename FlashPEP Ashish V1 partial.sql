...
if exists (select * from tempdb..sysobjects where name = 'NATChildWebMainPEP' and type = 'U')
...
where AdDate between getdate()-6 and getdate() and [Flash Entry Status] = 'JA Entry' and a.adtype in ('Electronic Circular','Email Promotion','Web Promotions')
Union
--For Coke----
select distinct OriginalFlyerId,FlyerID,a.Advertiser,AdDate,WeekOf,media,event,theme,a.market,Publication,salestartdate,saleenddate,pages,size,a.tradeclass,img,PublishedOn,Region,District,SalesTerritory,SalesTerritoryGroup,SalesTerritorySubGroup,TcUpdatedOn,DistUpdatedOn,RegionUpdatedOn,StUpdatedOn,StgUpdatedOn,StsgUpdatedOn,RetID,MktID,a.RetMktID,AdMonthid,AdDayid,CountryID,LanguageID,AVTCID,MediaID,ThemeID,EventID,subject,persona,receivedDt,AdSalePeriod,AdSaleWeek,AdSaleMonth,AdSaleYear,City,State,ZipCode,Gender,DoB,AgeBracket,DisplayMedia,Source,DistDt,AdDist,AdDistID,AdType,AdTypeID,Spend,Channel
from masteracfv.dbo.ACFVFlyerReports a with (nolock)
...
on a.retmktid = b.retmktid
where AdDate between getdate()-6 and getdate() and a.adtype in ('Electronic Circular','Email Promotion','Web Promotions')
go

if exists (select * from tempdb..sysobjects where name = 'NATPDTPEP' and type = 'U')
...
if exists(select * from tempdb..sysobjects where name = 'NATMPVPEP' and type = 'U')
...
if exists(select * from tempdb..sysobjects where name = 'FlashDataFlyerIDPEP' and type = 'U')
...
select * from mt2sql00.frnational.dbo.[NATChildWebMainPEP] a
join tempdb.dbo.[NATChildWebMainPEP] b
on a.originalflyerid = b.originalflyerid
go

insert into mt2sql00.frnational.dbo.[NATChildWebMainPEP] 
select * from tempdb.dbo.[NATChildWebMainPEP]
go
--select * into frnational..[NATChildWebMainPEP] from mt2sql13.tempdb.dbo.[NATChildWebMainPEP]
select count(*) from mt2sql00.frnational.dbo.[NATPDTPEP]  a
join tempdb.dbo.[NATPDTPEP] b
on a.originalpagedetailid = b.originalpagedetailid

select count(*) from mt2sql00.frnational.dbo.[NATPDTPEP]
...
on a.retmktid = b.retmktid
where AdDate between getdate()-6 and getdate() and [Flash Entry Status] = 'JA Entry' and adtype in ('Electronic Circular','Email Promotion','Web Promotions')
Union
...
on a.retmktid = b.retmktid
where AdDate between getdate()-6 and getdate() and adtype in ('Electronic Circular','Email Promotion','Web Promotions')
