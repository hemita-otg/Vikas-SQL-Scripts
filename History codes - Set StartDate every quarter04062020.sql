...
--Drop table masterclientcwm10142019

--Select * into masterclientcwm04062020 from mt2sql00.masterclientcoverage.dbo.masterclientcwm 
--End 
...
where clientid not in (3,12,46,92,95,98,146,147,162,999) and startdate<='03/31/2017' and StartDate<>DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
order by 1
*/
...
--/*
--Update M set StartDate='03/01/2013' ,EndDate='03/31/2016' 
...
from mt2sql00.masterclientcoverage.dbo.masterclientcwm M
where clientid in (3,12,46,92,95,98,146,147,162,999) and startdate <='03/31/2017'
...
--Drop Table ClientCWMException10142019
--select * from ClientCWMException
--select * from mt2sql14.sitecontrol.dbo.vw_catinfo where categorycodeid=102
/*
--Take backup of ClientCWMException table
select * into ClientCWMException04062020 from ClientCWMException
update x set startdate=DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
--select  startdate ,enddate ,DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
from clientcwmexception x where startdate <='03/31/2017' and  startdate<>DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
*/
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
