...
--Drop table masterclientcwmxxxxxxxx


--Select * into masterclientcwm01042016 from mt2sql00.masterclientcoverage.dbo.masterclientcwm 
--End 
...
where clientid not in (3,12,46,92,95,98,146,147,162,999) and startdate<='01/01/2013'
order by 1
*/
...
/*
Update M set StartDate='01/01/2010' ,EndDate='12/31/2012' 
...
--select distinct startdate ,enddate ,DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
from mt2sql00.masterclientcoverage.dbo.masterclientcwm M
where clientid in (3,12,46,92,95,98,146,147,162,999) and startdate <='01/01/2013'
...
--Drop Table ClientCWMExceptionxxxxxxxx

/*
--Take backup of ClientCWMException table
select * into ClientCWMException01042016 from ClientCWMException
update x set startdate=DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
--select distinct startdate ,enddate 
from clientcwmexception x where startdate <='01/01/2013'
*/
...
Create NonClustered Index IDX_PDTID On tempdb.dbo.PDTProcessedArchive (PageDetailId)
Go
Delete M   from MastserAdviewData.dbo.MasterMapPagedetailID M Join tempdb.dbo.PDTProcessedArchive T on M.OriginalPagedetailID=T.PagedetailId 
Go
...
select count(1) from mt3sql1.fvstaging.dbo.adviewmain--4731978
select count(1) from mt2sql00.masteradviewdata.dbo.adviewmain--4731874
select count(1) from mt2asql01.masteradviewdata.dbo.adviewmain --4731874 
select count(1) from mt4sql00.masterdata.dbo.adviewmain--4732403
