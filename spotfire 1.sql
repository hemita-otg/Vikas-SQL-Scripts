...
SET @SpotFireTableName = 'SPOTFIRE'
Select @ControlDBPrefix = dbo.mtfunc_GetDatabasePrefix('Control')
SET @NSQL = N'Select @IsFull = IsFull,@isINTID=isintegerid From  ' + @ControlDBPrefix + 'CategoryCode Where ID=' + cast(@CategoryCodeID as varchar)
execute sp_executesql @NSQL, N'@IsFull bit output,@isINTID bit output', @IsFull OUTPUT, @isINTID output
...
SET @SQL = ' Select '+ CASE WHEN @isINTID=1 THEN 'PDT.ORIGINALPageDetailID as PageDetailID' ELSE   'PDT.PageDetailID' END
+'''/ImageViewer.asp?BA8E341D95B8432D='' + '+CASE WHEN @isINTID=1 THEN 'PDT.ORIGINALPageDetailID as PageDetailID' ELSE   'PDT.PageDetailID' END+' + ''&AID='' as [Product Image],
...
' from MT2SQL15.SQL15.DBO.pgcorp50DetailReports pdt  '
