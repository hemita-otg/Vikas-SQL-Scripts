...
--Drop table masterclientcwm07012019

--Select * into masterclientcwm10142019 from mt2sql00.masterclientcoverage.dbo.masterclientcwm 
--End 
...
where clientid not in (3,12,46,92,95,98,146,147,162,999) and startdate<='10/01/2016' and StartDate<>DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
order by 1
*/
...
from mt2sql00.masterclientcoverage.dbo.masterclientcwm M
where clientid in (3,12,46,92,95,98,146,147,162,999) and startdate <='10/01/2016'
...
--Drop Table ClientCWMException07012019
...
select * into ClientCWMException10142019 from ClientCWMException
update x set startdate=DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
--select  startdate ,enddate ,DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
from clientcwmexception x where startdate <='10/01/2016' and  startdate<>DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
*/
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
