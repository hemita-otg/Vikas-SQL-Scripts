...
--Drop table masterclientcwm04062020

--Select * into masterclientcwm07062020 from mt2sql00.masterclientcoverage.dbo.masterclientcwm 
--End 
...
where clientid not in (3,12,46,92,95,98,146,147,162,999) and startdate<='04/01/2017' and StartDate<>DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
order by 1
*/
...
from mt2sql00.masterclientcoverage.dbo.masterclientcwm M
where clientid in (3,12,46,92,95,98,146,147,162,999) and startdate <='04/01/2017'
...
--Drop Table ClientCWMException04062020
...
select * into ClientCWMException07062020 from ClientCWMException
update x set startdate=DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
--select  startdate ,enddate ,DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
from clientcwmexception x where startdate <='04/01/2017' and  startdate<>DATEADD(yy, -3, DATEADD(qq, DATEDIFF(qq,0,getdate()), 0))
*/
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
