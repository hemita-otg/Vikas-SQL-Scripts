...
join masteradviewdata..adviewmain cwm on pdt.flyerid=cwm.flyerid and cwm.addate >= '01/01/2009' 
...
alter table tempdb..adamscounts  add Ear_Entered_AdDate datetime
...
join tempdb..adamscounts c on cwm.retmktid=c.retmktid and cwm.addate >= '01/01/2009' 
group by c.retmktid
...
-- Regular count
update c set Ear_Entered_AdDate=x.dt
--select c.retmktid, category, x.cnt-- to cross check before update
from tempdb..adamscounts c
inner join 
(
select c.retmktid, c.category, min(addate) as dt
from masteradviewdata.. Masterpagedetailtable pdt 
join masteradviewdata..adviewmain cwm on pdt.flyerid=cwm.flyerid 
join tempdb..adamscounts c on pdt.category=c.category and cwm.retmktid=c.retmktid and cwm.addate >= '01/01/2009' 
group by c.retmktid, c.category
) x on c.retmktid=x.retmktid and c.category=x.category
-----------------------------------------------------------------
